...
class WorkOrderController extends AppController
{
    public function pengiriman($id)
    {
        ...
        try
        {
            ...
            if($data->status_pengiriman == 0 && $data->jenis_wo == 1){ // Cek jika WO hanya PO
    ...
    public function index(Request $request)
    {
        if($request->ajax())
        {
            ...
            $datatables = Datatables::of($query)
                ...
                ->editColumn('sisa', function($value){
                    return toRp($value->sisa);
                  })
                ->rawColumns(['action']);
    ...
    public function store(Request $request)
    {
        ...
        if(count(array($keteranganPembayaran)) == 0){
            notify()->flash('Error!', 'error', [
        ...
        try
        {
            ...
            for ($i=0; $i < count(array($keteranganPembayaran)); $i++) {
